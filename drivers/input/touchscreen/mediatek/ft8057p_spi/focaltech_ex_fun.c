...
// Use num_read_chars to avoid warning
if (num_read_chars > 0) {
// Code here
}
    ...
    mutex_unlock(&input_dev->mutex);
    update_lct_tp_info(buf, tp_lockdowninfo);

    FTS_INFO("tp_info:%s", buf);
...
int lct_fts_tp_proximity_callback(bool enFlag)
{
    int ret = 0;

    ret = fts_set_proximity_switch(enFlag);
    if(ret < 0)
    {
        FTS_INFO("Set fts_set_proximity_switch fail!, enFlag=%d\n", enFlag);
    }
    FTS_INFO("Set fts_set_proximity_switch success, flag=%d\n", enFlag);
